...
function arrival(session) {
    if (cachedArrival && cachedSession === session)
        return cachedArrival;
    let saved;
    try {
        saved = JSON.parse(window.localStorage.getItem(ARRIVAL_KEY) || 'null');
    }
    ...
    cachedArrival = session && saved?.session === session && a &&
        ...
        ['utm_source', 'utm_medium', 'utm_campaign'].every(k => typeof a[k] === 'string' && (a[k] === '' || a[k] === campaign(a[k])))
        ? Object.fromEntries(['landing_path', 'referrer_origin', 'arrival_type',
            'utm_source', 'utm_medium', 'utm_campaign'].map(k => [k, a[k]]))
        : freshArrival();
    try {
        window.localStorage.setItem(ARRIVAL_KEY, JSON.stringify({ session, arrival: cachedArrival }));
    }
    catch { /* No storage must never break the lab. */ }
    return cachedArrival;
}
/** Runs after SDK enrichment, immediately before transport. Never persist raw URLs. */
export function sanitizePayload(event) {
    if (!event || new URLSearchParams(window.location.search).get('verification') === 'controlled')
        return null;
    if (event.event !== '$pageview' && !/^(lab_|reaction_|chemistry_guide_)/.test(event.event))
        return null;
